...
export async function encryptMessageHybrid(
  ...
  if (senderPubKey) {
    const selfEncrypted = await crypto.subtle.encrypt(
      { name: "RSA-OAEP" },
      senderPubKey,
      rawAesKey,
    );
    encryptedKeyForSelf = btoa(
      String.fromCharCode(...new Uint8Array(selfEncrypted)),
    );
  }
...
export async function decryptMessageHybrid(
  ...
  try {
    const encKeyB64 =
      isSent && payload.encryptedKeyForSelf
        ? payload.encryptedKeyForSelf
        : payload.encryptedKey;

    const encryptedAesKey = Uint8Array.from(atob(encKeyB64), (c) =>
      c.charCodeAt(0),
    );
