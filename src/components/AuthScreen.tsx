...
type KeyStepState = { id: string; label: string; icon: React.ReactNode; status: StepStatus }[];
...
export default function AuthScreen() {
  ...
  async function handleLogin() {
    ...
    setLoading(true);
    try {
      // POST to our Next.js route — sets HttpOnly cookie server-side
      ...
      // Token is now in HttpOnly cookie. We only get back non-sensitive user info.
      const user: User = { id: data.user_id, username: loginUsername };
      dispatch({ type: 'SET_USER', user });

      // Save username for session restore (not sensitive)
  ...
  async function handleRegister() {
    ...
    setLoading(true);
    try {
      const { keyPair, wrappedPrivateKey, saltB64 } = await setupNewKeys(regUsername, regPassword);
      ...
      dispatch({ type: 'SET_USER', user });
      dispatch({ type: 'SET_KEYS', privateKey: keyPair.privateKey, publicKey: keyPair.publicKey });
  ...
  async function afterLogin(user: User, password: string) {
    try {
      // Fetch full profile via proxy (uses HttpOnly cookie)
      const res  = await fetch(`/api/proxy?path=${encodeURIComponent('/auth/me')}`);
      const me   = await res.json() as User;
      const fullUser: User = { ...user, ...me, id: me.id || me.user_id || user.id };
      ...
      if (privateKey) {
        if (fullUser.public_key) {
          try { publicKey = await importPublicKey(fullUser.public_key); } catch { /* ok */ }
        }
      } else if (fullUser.wrapped_private_key && fullUser.pbkdf2_salt) {
        // Restore from server-wrapped key
        const { unwrapPrivateKey, deriveWrappingKey: dwk } = await import('@/lib/crypto');
        const wrappingKey = await dwk(password, fullUser.pbkdf2_salt);
        privateKey = await unwrapPrivateKey(fullUser.wrapped_private_key, wrappingKey);
        await savePrivateKey(db, fullUser.username, privateKey);
        if (fullUser.public_key) {
          try { publicKey = await importPublicKey(fullUser.public_key); } catch { /* ok */ }
        }
      } else {
        // Generate fresh keys on new device
        const { keyPair, wrappedPrivateKey, saltB64 } = await setupNewKeys(fullUser.username, password);
        privateKey = keyPair.privateKey;
        publicKey  = keyPair.publicKey;
        const pubKeyB64 = await exportKey(publicKey, 'spki');
        // Try to upload new public key
        await fetch(`/api/proxy?path=${encodeURIComponent('/users/me/public-key')}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ public_key: pubKeyB64, wrapped_private_key: wrappedPrivateKey, pbkdf2_salt: saltB64 }),
        }).catch(() => {});
      }
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-[#0f0f0f] z-[100]">
        <div className="w-90 px-5 py-10 text-center">
          ...
          <p className="mt-8 text-sm text-[#707579]">Your messages are end-to-end encrypted.</p>
        </div>
      </div>
    </>
  );
